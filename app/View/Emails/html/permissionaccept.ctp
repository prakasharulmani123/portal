          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td width="80%" align="left" valign="top"><font style="font-family: Georgia, 'Times New Roman', Times, serif; color:#010101; font-size:24px"><strong><em>Dear <?php echo $user['User']['employee_name']?>,</em></strong></font><br /><br />
				  ...
				  if($status == 1){
					echo 'Your Permission Request on '.'&nbsp;'.date('d-m-Y',strtotime($permission['Permission']['date'])).' (From '.date('h:i A', strtotime($permission['Permission']['from_time'])).' to '.date('h:i A', strtotime($permission['Permission']['to_time'])).' - '.gmdate("H:i", ($hours* 60)).' Hours'.') Accepted '.'<br />'.$permission['Permission']['remarks'];
				  }
				  elseif($status == 2){
					echo 'Your Permission Request '.'&nbsp;'.date('d-m-Y',strtotime($permission['Permission']['date'])).' (From '.date('h:i A', strtotime($permission['Permission']['from_time'])).' to '.date('h:i A', strtotime($permission['Permission']['to_time'])).' - '.gmdate("H:i", ($hours* 60)).') Declined'.'<br />'.$permission['Permission']['remarks'];
				  }
				  ?>
<br /><br />
