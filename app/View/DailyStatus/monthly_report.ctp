...
<?php $late_entries = $this->requestAction('late_entries/get_late_entry_by_user_id/' . $this->Session->read('User.id')); ?>
<?php $late_fee = 0; ?>
...
<?php $user_complaints = $this->requestAction('user_complaints/get_user_their_complaint_by_user_id/' . $this->Session->read('User.id')); ?>
<?php $fine = 0; ?>
...
<?php $my_complaints = $this->requestAction('user_complaints/get_user_my_complaint_by_user_id/' . $this->Session->read('User.id')); ?>
<?php $earnings = 0; ?>
...
<div style="margin:10px 50px 10px 50px;" id="month-div">
    ...
    ?>
    <b><?php echo " Month : " ?></b>
    <?php echo $this->Form->input('month', array('label' => false, 'div' => false, 'class' => 'form-control', 'value' => $value, 'style' => 'width:200px; margin-top:6px;', 'data-calendar' => 'false')); ?>
    <?php echo $this->Form->button('Search', array('class' => 'btn btn-default')); ?></td>
    <?php echo $this->Html->link('Reset', array('controller' => 'dailystatus', 'action' => 'reset_month', 'admin' => false), array('class' => 'btn btn-danger')); ?>
				...
				<tr align=left><th>Su</th><th>M</th><th>Tu</th><th>W</th><th>Th</th><th>F</th><th>Sa</th></tr>";
                                ...
                                if ($check_working_day == true) {
                                    if ($check_permission == true) {
                                        $permission_row = $this->requestAction('permission/get_permission_by_userid_date/' . $this->Session->read('User.id') . '/' . $dailyreport);
                                    ...
                                    <tr>
                                        ...
                                        <td <?php echo $color != '' ? $color : '' ?>class="ttRC" title="<?php echo $txt ?>"><?php echo date('d-m-Y', strtotime($dailyreport)); ?><?php echo $check_permission == true ? '&nbsp;(Permission : ' . gmdate("H:i", ($hours * 60)) . ' hour)' : '' ?> <?php echo $check_leave == true ? '&nbsp;(Half a day Leave)' : '' ?></td>
                                        ...
                                        <td <?php echo $color != '' ? $color : '' ?>><a href="<?php echo $this->base ?>/dailystatus/reports_view/<?php echo $reports[0]['DailyStatus']['id'] ?>" title="View Report"><span class="isb-text_document"></span></a></td>
                                    </tr>
        ...
        if ($check_permission == true && $permission_showed == false) {
            ...
            ?>
                                    <tr>
                                        ...
                                        <td style="background-color:pink"class="ttRC" title="<?php echo $txt ?>"><?php echo date('d-m-Y', strtotime($dailyreport));
                        echo '&nbsp;(Permission : ' . gmdate("H:i", ($hours * 60)) . ' hour)'; ?></td>
            ...
            $no_record_count++;
                                ...
                                <?php if ($check_leave == true) { ?>
                                    <?php $leave_row = $this->requestAction('leave/get_leave_by_userid_date/' . $this->Session->read('User.id') . '/' . $dailyreport); ?>
                                    <?php
